...
**Answer 8:**
After comparing the differing year values, I can see that as the range of years extends the rate at which the value of the CO2 concentration changes. This could be caused because of a larging range of data taking out the possiblilties of pauses in the data.
```{r}
q3_long <- q3 |> 
  pivot_longer(cols = c(`5-year running average`, `10-year running average`, `20-year running average`)) 
  
ggplot(q3_long, aes(Year, value, color = name)) + geom_line()
```
**Question 9:**
...
```{r}
q10 <- read_csv("vostok.icecore.co2")
q10 <- q10 |> 
  mutate("Depth (m)" = 149.1)
```
