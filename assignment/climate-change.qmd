...
**Answer 1:**
The overall trend for the CO2 data is a linear progression with small declining influxes every year.
...
**Answer 2:**
Within a year, CO2 increases and decreases around the same time. The month with the max CO2 is typically May and the min month is September. Something that may explain this sub-annual pattern could be the use of fossil fuels for heating in the colder months. During the summer months the CO2 concentrations drop due to the lessening use of heating, but during the colder months, heating systems turn back on and the CO2 concentrations increase.

```{r}
co2 |> 
  group_by(month) |> 
  summarize(total_monthly_average = mean(monthly_average, na.rm = TRUE)) |> 
  ggplot(aes(month, total_monthly_average)) + geom_line()
```
...
**Answer 3:**
This data is from NASA's Goddard Institute for Space Studies.

The Year column is a integer and its measured in years. The No-Smoothing column is numeric and its measured in units of Celcius.The column Lowess(5) is numeric and it is measured in Celcus.

The difference between global temperature and global temperature anomly is that global temperature just shows the average temperature around the globe where global temperature anomaly shows how much warmer than average was the most recent year was.
**Question 4:**
...
**Answer 4:**
```{r}
file_url <- "https://data.giss.nasa.gov/gistemp/graphs/graph_data/Global_Mean_Estimates_based_on_Land_and_Ocean_Data/graph.txt"

q3 <- read_table(file_url, skip = 5, col_names = c("Year", "Mean_Average", "Lowess"))
```
**Question 5:**
...
In the plot, the global mean temperature remained relatively the same until an steady increase started from 1970 - 2024. I believe the influx of human activities like fossil fuel use caused the continuing spike since 1970 causing this global temperature to rise due to climate change and global warming.
```{r}
q3 |> 
  ggplot(aes(Year, Mean_Average)) + geom_line() + geom_point(color = "red")
```
...
**Answer 6:**
I used the ChatGPT and the prompt used was "How do I calculate rolling average in R". From the chat LLM I used library(zoo) and the rollmean() function. From the chat LLM I had to modify the answer by taking out examples that it provided for using the code.
...
**Answer 7:**
The annual average shows just one year of data whereas the 5-year average shows an average value over fives years and smooths out the data to take away short term variation and to better identify patterns
```{r}
install.packages("zoo", repos = "https://cloud.r-project.org/")
library(zoo)
q3 <- q3 |> 
  mutate("5-year running average" = rollmean(Mean_Average, k = 5, fill = NA)) |> 
  mutate("10-year running average" = rollmean(Mean_Average, k = 10, fill = NA)) |> 
  mutate("20-year running average" = rollmean(Mean_Average, k = 20, fill = NA))
```
**Question 8:**
...
**Answer 8:**
After comparing the differing year values, I can see that as the range of years extends the rate at which the value of the CO2 concentration changes. This could be caused because of a large range of data taking out the possiblilties of pauses in the data and extreme dips from certain months and years.
```{r}
q3_long <- q3 |> 
  pivot_longer(cols = c(`5-year running average`, `10-year running average`, `20-year running average`)) 
  
ggplot(q3_long, aes(Year, value, color = name)) + geom_line() + labs(title = "5, 10, and 20-year running Average", x = "Year", y = "Mean Average")
```
**Question 9:**
...
**Answer 9:**
When looking at the above graph, examining the 5 and 10-year averages, you can still see the heavy increases in the data shown. When looking at the 20-year running averages, the evidence of pauses become clearer as the there are moments when the rate of the increase becomes stagnant, then years later it resumes and continues to increase.
...
The data set shows the historical CO2 record from the Vostok Ice Core. The columns are Depth (m), Age of Ice (Year, BP), Mean age of the air (Year BP), and CO2 Concentration (ppmv). The numbers come from research from the Laboratoire de Glaciologie et de Geophysique de l'Environnement and Arctic and Antarctic Research Institute. The temporal resolution of this data is January 2003. After looking at the Vostok Ice and Moana Loa graph I conclude that there is a clear jump in atmospheric CO2 levels from historic levels. When looking at the plot, the last measure of the historic data from the Vostok Ice Core is at a CO2 level of 277.6 then increases to 315.2370.
```{r}
q10 <- read_table("vostok.icecore.co2", col_names = c("Depth (m)", "Age of the Ice (year BP)", "Mean age of the air (year BP)", "CO2 concentration (ppmv)"), col_types = cols(.default = "c"), skip = 20)

q10 <- q10 |> 
  mutate(`Age of the Ice (year BP)` = as.numeric(`Age of the Ice (year BP)`), 
      `Mean age of the air (year BP)` = as.numeric(`Mean age of the air (year BP)`),
      `CO2 concentration (ppmv)` = as.numeric(`CO2 concentration (ppmv)`)) |> 
      arrange(`Age of the Ice (year BP)`)

q10 |> 
  ggplot(aes(`Age of the Ice (year BP)`, `CO2 concentration (ppmv)`)) + geom_line(color = "red") + labs(title = "Vostok Ice Core Data", x = "Age of the Ice (year BP)", y = "CO2 Concentration (ppmv)") + theme_gray()

co2_q10 <- co2 |> 
  group_by(year) |> 
  summarize(total_yearly_average = mean(monthly_average, na.rm = TRUE)) |> 
  mutate(`Age of the Ice (year BP)` = 1950 - year)

q10_c02_binded <- q10 |> 
  bind_rows(co2_q10) |> 
  arrange(desc(`Age of the Ice (year BP)`))

Final_Combined_CO2_Data <- q10_c02_binded |> 
  mutate(`CO2 Concentration (ppmv) Combined` = coalesce(`CO2 concentration (ppmv)`, `total_yearly_average`))

Final_Combined_CO2_Data |> 
  ggplot(aes(`Age of the Ice (year BP)`, `CO2 Concentration (ppmv) Combined`)) + 
  geom_line(color = "blue") + 
  labs(title = "Vostok Ice Core and Mauna Loa CO2 Data", 
       x = "Age of the Ice (year BP)", 
